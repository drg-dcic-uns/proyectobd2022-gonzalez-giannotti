...
public class DAOVuelosImpl implements DAOVuelos {
	...
	public ArrayList<InstanciaVueloBean> recuperarVuelosDisponibles(Date fechaVuelo, UbicacionesBean origen,
		...
		funciona bien tu version pero no funciona con mi recuperarDetalle() no se porque !
		...
		String sql = "select v.nro_vuelo,v.dia_sale,v.hora_sale,v.hora_llega,v.tiempo_estimado,v.modelo,v.fecha,"
				...
				+ " ubicaciones us on pais_sale= us.pais and estado_sale = us.estado and ciudad_sale = us.ciudad  WHERE v.fecha= '"+fecha+"' and v.ciudad_sale= '"+origen.getCiudad()+"' and "
					+ " v.estado_sale= '"+origen.getEstado()+"' and v.pais_sale= '"+origen.getPais()+"' and v.ciudad_llega= '"+destino.getCiudad()+"' and "
							+ "v.estado_llega= '"+destino.getEstado()+"' and v.pais_llega= '"+destino.getPais()+"';"; 
		try
		{       
	...
	public ArrayList<DetalleVueloBean> recuperarDetalleVuelo(InstanciaVueloBean vuelo) throws Exception {
		...
		String sql = "select clase, precio, asientos_disponibles from vuelos_disponibles where nro_vuelo= '"+vuelo.getNroVuelo()+"' and "
				+ " fecha= '"+fechaVuelo+"' ;";
